...
class CI_DB_oci8_driver extends CI_DB {
	...
	var $stmt_id;
	var $curs_id;
    var $cur_ref = array('cur','cr','cursor','refcursor',SQLT_RSET);
    var $myrefcur = array();
	...
	protected function _execute($sql)
	{
        ...
        if (is_array($myarray)) {    
            foreach ($myarray as $value)     { 
            	if ($value['type'] === 50 )
			{
		        $this->clob = oci_new_descriptor($this->conn_id, OCI_D_LOB);
			} 
                if ($value['type'] ===  116 )
			{
		        $this->myrefcur[$i] = $this->get_cursor();
                $i++;
			}  
			  if ($value['type'] ===  113 )
			{
        ...
        $exec_worked = ociexecute($this->stmt_id, OCI_DEFAULT);
 ...
 $cursorCounter = 0;
		  ...
		  oci_execute($this->myrefcur[$cursorCounter], OCI_DEFAULT);

          oci_fetch_all($this->myrefcur[$cursorCounter], $this->myrefcur[$cursorCounter], null, null,OCI_FETCHSTATEMENT_BY_ROW);
$cursorCounter ++;
...
//oci_execute($this->stmt_id, OCI_DEFAULT);
		...
		//	echo $value['value'];	
            ...
            oci_commit($this->conn_id);    
		       ...
		       echo $this->lob->load();
       			//file_put_contents($this->file, $this->lob->load());
	...
	public function get_cursor()
	{
		...
		//return $this->curs_id;
        return oci_new_cursor($this->conn_id);
	}
...
$package.=".";
		...
		foreach ($params as $param)
		{
			...
			if (array_key_exists('type', $params) && ($param['type'] === OCI_B_CURSOR))
			{
				$have_cursor = TRUE;
			}
		}
    ...
    $sql = trim($sql, ", ");
     ...
     return  $this->query($sql, $params, $have_cursor);
...
$i=0;
		foreach ($params as $param)
		{
			foreach (array('name', 'value', 'length', 'type') as $val)
			{
				if ( ! isset($param[$val]))
				{
					$param[$val] = '';
...
//echo   $param['name']."   ".$param['value']."   ". $param['length']."   ".$param['type'];
			if ($param['type'] === 50)
			{//echo "50";
	 
oci_bind_by_name($this->stmt_id , $param['name'], $this->clob, $param['length'], SQLT_CLOB);
$this->clob->writetemporary($param['value']);
...
$this->clob->writetemporary($param['value']);
            ...
            else if ($param['type'] === 116)
			{//echo "116";
            
oci_bind_by_name($this->stmt_id , $param['name'], $this->myrefcur[$i], $param['length'], OCI_B_CURSOR);
        $i++;	
            }
            
            
			else if ($param['type'] === 113)
{
oci_bind_by_name($this->stmt_id , $param['name'], $this->lob, $param['length'], OCI_B_BLOB);
//$this->lob->savefile($param['value']);   /************modified************/
}
